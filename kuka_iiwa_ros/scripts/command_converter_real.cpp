...
public:
  ...
  SubscribeAndPublish()
  {
    ...
    this->cmd_left = {0,0,0,0,0,0};
    this->cmd_right = {0,0,0,0,0,0};
    this->max_vel = 0.2;
  }
  ...
  void callback_position_left(const geometry_msgs::TwistStamped::ConstPtr& msg)
  {
    ...
    for (int i=3;i<6;i++)
    {
      norm+=cmd_left[i]*cmd_left[3];
    }

    norm=sqrt(norm);
    if (norm>this->max_vel)
    {
        this->cmd_left[3] = this->cmd_left[3]/norm*this->max_vel;
        this->cmd_left[4] = this->cmd_left[4]/norm*this->max_vel;
        this->cmd_left[5] = this->cmd_left[5]/norm*this->max_vel;
    }
  ...
  void callback_position_right(const geometry_msgs::TwistStamped::ConstPtr& msg)
  {
    ...
    for (int i=3;i<6;i++)
    {
      norm+=cmd_right[i]*cmd_right[3];
    }

    norm=sqrt(norm);
    if (norm>this->max_vel)
    {
        this->cmd_right[3] = this->cmd_right[3]/norm*this->max_vel;
        this->cmd_right[4] = this->cmd_right[4]/norm*this->max_vel;
        this->cmd_right[5] = this->cmd_right[5]/norm*this->max_vel;
    }
